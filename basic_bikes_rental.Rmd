...
```{r}
#testing <- read.table("data/test.csv", na.strings="", header=TRUE, sep=";", dec=".") %>%
testing <- read_delim("data/test.csv", delim = ";", col_names = TRUE) %>%
  select(-id)
head(testing)
```
...
```{r}
# Outliers: humidity
boxplot(testing$humidity, horizontal = TRUE)
boxplot.stats(testing$humidity)

testing_without_liers <- testing$humidity[testing$humidity > 0]

# Comprobación
boxplot(testing_without_liers, horizontal = TRUE)
boxplot.stats(testing_without_liers)

# Filtrado de dataframe
bikes_testing_set <- filter(testing, testing$humidity > 0)

str(bikes_testing_set)
head(bikes_testing_set)
```
...
```{r}
predict(model_rf_reg, newdata = head(bikes_testing_set))
predict(model_xgbTree, newdata = head(bikes_testing_set))
predict(model_glmnet_reg, newdata = head(bikes_testing_set))
```
