...
train_bikes_boxplot <- select(train_bikes, -year, -num_bikes)
boxplot(train_bikes_boxplot, col = rainbow(ncol(train_bikes_boxplot)))
```

```{r}
# First half
train_bikes_half1 <- select(train_bikes, year, hour, season, holiday, workingday, weather)
par(mfrow = c(1, ncol(train_bikes_half1)))
invisible(lapply(1:ncol(train_bikes_half1), function(i) boxplot(train_bikes_half1[, i])))

# Second half
train_bikes_half2 <- select(train_bikes, temp, atemp, humidity, windspeed, num_bikes)
par(mfrow = c(1, ncol(train_bikes_half2)))
invisible(lapply(1:ncol(train_bikes_half2), function(i) boxplot(train_bikes_half2[, i])))
```
...
```{r}
# Outliers: holiday
boxplot(train_bikes$holiday, horizontal = TRUE)
boxplot.stats(train_bikes$holiday)
```
