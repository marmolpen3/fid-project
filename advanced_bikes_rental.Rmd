...
Instalación de ***tidyverse*** para la manipulación de los datos y la visualización, de ***Visadat*** para búsqueda de valores nulos, de ***ggplot2*** para visualización, de **corrplot** para la correlación de las variables y de **NbClust** y **factoextra** para clustering, por último, de **ape** para la representación de los dendogramas.
...
contry_data_clust_df <- cbind(country_data_df, sample_c_clusters_df)

mean_data_complete <- contry_data_clust_df %>%
...
```{r}
sample_c_clusters <- cutree(hier_clust_c_single, k = 3)

colors = c("red", "blue", "green")
plot(as.phylo(hier_clust_c_single), type = "fan", tip.color = colors[sample_c_clusters],
     label.offset = 1, cex = 0.5)
```
...
```{r}
contry_data_clust_df[contry_data_clust_df$sample_c_clusters == 3]
```

#### Conclusiones

Si observamos los resultados obtenidos aplicando los diferentes modelos podemos deducir que el **Clustering Jerárquico** con método **Single** es el que peores resultados obtiene, puesto que dos de los clusters contienen un único ejemplo, aspecto que no consideramos correcto porque existen bastantes países con baja calidad de vida en el dataset.

Sin embargo, tanto el algoritmo **k-means** como el **Clustering Jerárquico** con método **Complete** consiguen unos resultados con mayor calidad.
